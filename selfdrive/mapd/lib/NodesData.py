...
def speed_limits_for_curvatures_data(curv, dist):
  ...
  # Define a function to map the curvature sections data arrays into turn speed sections data arrays.
  def speed_section_data(curv_sec):
    max_curv_idx = np.argmax(curv_sec[:, 0])
    start = np.amin(curv_sec[:, 2])
    end = np.amax(curv_sec[:, 2])
    # provide [section start distance, section end distance, speed limit based on max curvature, sing of curvature]
  ...
  return np.array([speed_section_data(sd) for sd in curv_secs])
